	...
	for (int i = 0; i < numStripes; i++)
	{
		...
		else
		{
			...
			for (std::vector<IPvXAddress>::iterator it = children.begin() ; it != children.end(); ++it)
			{
				...
				IPvXAddress alternativeParent = getAlternativeNode(stripe, addr);
...
void MultitreePeer::connectVia(IPvXAddress address, const std::vector<int> &stripes)
{
	...
	// TODO set for each stripe individually
	pkt->setLastReceivedChunk(getGreatestReceivedSeqNumber());
